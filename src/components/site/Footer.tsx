    ...
    <footer className="border-t border-border/60 bg-secondary/40">
      <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 max-w-md">
          <Link to="/" className="flex items-center gap-2.5" aria-label="AllWordTools home">
        ...
        <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              {footerPopular.map((tool) => (
                <li key={tool.slug}>
                  <Link to="/tool/$tool" params={{ tool: tool.slug }} className={muted}>
                    {tool.name}
                  </Link>
                </li>
              ))}
              <li>
                <Link to="/tools" className="font-semibold text-honey hover:underline">
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              {categories.slice(0, 7).map((cat) => (
                <li key={cat.slug}>
                  <Link to="/category/$category" params={{ category: cat.slug }} className={muted}>
                    {cat.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              ...
              <li>
                <Link to="/category/$category" params={{ category: "ai-tools" }} className={muted}>
                  {t("footer.aiTools")}
                </Link>
              </li>
              <li>
                <Link to="/tool/$tool" params={{ tool: "word-of-the-day" }} className={muted}>
                  {t("footer.wordOfDay")}
                </Link>
              </li>
              <li>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2.5 text-sm">
              ...
              <li>
                <Link to="/cookie-policy" className={muted}>
                  {t("footer.cookiePolicy")}
                </Link>
              </li>
              <li>
                <Link to="/disclaimer" className={muted}>
                  {t("footer.disclaimer")}
                </Link>
              </li>
              <li>
