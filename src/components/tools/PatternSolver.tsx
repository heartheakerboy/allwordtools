...
export function PatternSolver() {
  ...
  const handleSolve = () => {
    if (!pattern.trim()) {
      toast.error("Enter a pattern with known letters and blanks.");
      return;
    }
  ...
  return (
    <div className="rounded-3xl border border-border/70 bg-card p-5 shadow-lift sm:p-7">
      <div className="space-y-1.5">
        <Label htmlFor="ps-pattern" className="text-xs font-semibold text-muted-foreground">
          Pattern (use a letter for what you know, ? _ or . for each blank)
        </Label>
        <div className="flex flex-col gap-3 sm:flex-row">
          <Input
            ...
            placeholder="e.g. c?t, ap?le, ?? zzle"
            autoComplete="off"
            spellCheck={false}
      ...
      <p className="mt-3 text-xs text-muted-foreground">
        Each blank stands for exactly one letter, so the pattern length equals the word length. Use{" "}
        <code className="font-mono">?</code>, <code className="font-mono">_</code> or{" "}
