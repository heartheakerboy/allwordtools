...
export function LetterPatternFinder() {
  const { words, loading, error } = useDictionary();
  const [pattern, setPattern] = useState("");
  const [length, setLength] = useState(0);
  const [submitted, setSubmitted] = useState<{ pattern: string; length: number } | null>(null);

  const results = useMemo(() => {
    if (!words || !submitted) return null;
    return matchPattern(submitted.pattern, words, submitted.length, 800);
  }, [words, submitted]);
    ...
    <div className="rounded-3xl border border-border/70 bg-card p-5 shadow-lift sm:p-7">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="space-y-1.5 sm:col-span-2">
          <Label htmlFor="pf-pattern" className="text-xs font-semibold text-muted-foreground">
            Pattern (? = one letter, * = any run of letters)
          </Label>
          <Input
            id="pf-pattern"
            value={pattern}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="pf-length" className="text-xs font-semibold text-muted-foreground">
            Exact length
          </Label>
          <select
            id="pf-length"
      ...
      <p className="mt-3 text-xs text-muted-foreground">
        Use fixed letters for what you know, <code className="font-mono">?</code> for a single
        unknown letter, and <code className="font-mono">*</code> for any number of letters.
      </p>
        ...
        <div className="mt-7 border-t border-border/60 pt-6">
          ...
          {results.length === 0 ? (
            <p className="mt-4 text-sm text-muted-foreground">
              No words match that pattern. Try replacing a fixed letter with ? or *.
            </p>
          ) : (
