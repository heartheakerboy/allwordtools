...
export function TextTwistSolver() {
  ...
  const results = useMemo(() => {
    ...
    return unscramble(submitted, words, { minLength: 3 }, 800);
  }, [words, submitted]);
  ...
  const handleSolve = () => {
    ...
    if (cleaned.length < 3) {
      toast.error("Enter at least three letters.");
      return;
    }
    ...
    <div className="rounded-3xl border border-border/70 bg-card p-5 shadow-lift sm:p-7">
      <div className="flex flex-col gap-3 sm:flex-row">
        <div className="relative flex-1">
          ...
          <Input
            value={letters}
            onChange={(e) => setLetters(e.target.value.slice(0, 12))}
            onKeyDown={(e) => e.key === "Enter" && handleSolve()}
            placeholder="Enter your scrambled tiles"
